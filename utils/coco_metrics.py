...
def coco_map_cpu(res_file, ann_file, ann_type='bbox', silence=True):
    """
    :param res_file: predict list or predict json path
    :param ann_file: gt dict or gt json path
    :param ann_type: bbox
    :param silence:
    :return:
    """
    ...
    stats_names = ['AP', 'AP@.5', 'AP@.75', 'AP(S)', 'AP(M)', 'AP(L)',
...
def __ap_per_class(tp, conf, pred_cls, target_cls):
    """ Compute the average precision, given the recall and precision curves.
    ...
    for ci, c in enumerate(unique_classes):
        ...
        if n_p == 0 or n_gt == 0:
            ...
            for j in range(tp.shape[1]):
                ap[ci, j] = __compute_ap(recall[:, j], precision[:, j])
...
def coco_map_tensor(predicts_list, targets_list):
    """
    ...
    if len(stats):
        p, r, ap, f1, ap_class = __ap_per_class(*stats)
